      ...
      <tr>
        ...
        {#each standings.problems as prob}
          <th class="py-3.5 px-4 text-center min-w-[70px]">
            <a href={`/problems/${prob.problemId}`} class="hover:text-indigo-400 font-bold text-base transition">
              {prob.label}
            </a>
          </th>
        {/each}
      ...
      {#each standings.standings as participant}
        <tr class="hover:bg-zinc-800/30 transition {participant.rank === 1 ? 'bg-amber-500/5' : ''}">
          ...
          <td class="py-3 px-4 font-semibold text-zinc-100 flex items-center space-x-2">
            <div class="w-6 h-6 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs text-indigo-400">
              {participant.username.slice(0, 1).toUpperCase()}
            </div>
            <span>{participant.username}</span>
          </td>

          <td class="py-3 px-4 text-center font-bold text-base text-indigo-400 font-mono">
          ...
          {#each standings.problems as prob}
            ...
            <td class="py-3 px-4 text-center">
              {#if pScore && pScore.solved}
                <div class="p-1.5 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-mono text-xs font-bold">
              ...
              {:else if pScore && pScore.attempts > 0}
                <div class="p-1.5 rounded-lg bg-rose-500/20 border border-rose-500/40 text-rose-300 font-mono text-xs font-bold">
                  -{pScore.attempts}
                </div>
              {:else}
